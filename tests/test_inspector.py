...
def get_trained_inspector(model, X,y):
    test_inspector = InspectorShap(model,n_clusters=4)
    test_inspector.inspect(X,y, approximate=True)
    return tes
...
def test_inspector_with_eval_set():
    ...
    # Make sure the assertion works ehrn the sampple names length does not metch the eval set lenght
